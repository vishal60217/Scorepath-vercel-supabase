...
app.get('/api/health',async(q,r)=>{
  ...
  let counts=null;
  if(healthy && hasPostgres){try{const rows=await sql('SELECT (SELECT COUNT(*) FROM courses)::int AS courses,(SELECT COUNT(*) FROM lessons)::int AS lessons,(SELECT COUNT(*) FROM plans)::int AS plans,(SELECT COUNT(*) FROM teachers)::int AS teachers,(SELECT COUNT(*) FROM classes)::int AS classes,(SELECT COUNT(*) FROM questions)::int AS questions');counts=rows[0];}catch(e){counts={error:e.message};}}
  const detail = startupError ? { code: startupError.code || startupError.name || 'DB_INIT_ERROR', message: process.env.NODE_ENV === 'production' ? 'Database initialization failed. Check DATABASE_URL and Supabase connection settings.' : startupError.message } : null;
