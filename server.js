...
if (hasPostgres) {
  ...
  pgPool = new Pool({ connectionString: databaseUrl, ssl: process.env.DATABASE_SSL === 'false' ? false : { rejectUnauthorized: false }, max: 2, connectionTimeoutMillis: 8000, idleTimeoutMillis: 20000, keepAlive: true, allowExitOnIdle: true });
  ...
  if(!startupPromise){
    ...
    startupPromise=(async()=>{
      ...
      for(let attempt=1;attempt<=3;attempt++){
        try{if(hasPostgres)await initPostgres();await ensureAdmin();startupError=null;console.log(`ScorePath ready (${hasPostgres?'PostgreSQL':'local development storage'})`);return;}
        catch(e){last=e;startupError=e;console.error(`Startup attempt ${attempt}/3 failed:`,e.message);if(attempt<3)await new Promise(r=>setTimeout(r,400*attempt));}
      }
      throw last;
    })().catch(e=>{startupError=e;throw e;});
  }
  return startupPromise;
}
app.get('/api/health',async(q,r)=>{
  let dbOk=!hasPostgres, error=null, counts=null;
  if(hasPostgres){try{await sql('SELECT 1');dbOk=true;}catch(e){error={code:e.code||e.name||'DB_ERROR',message:process.env.NODE_ENV==='production'?'Database connection failed. Check DATABASE_URL and Supabase connection settings.':e.message};}}
  if(dbOk&&hasPostgres){try{const rows=await sql('SELECT (SELECT COUNT(*) FROM courses)::int AS courses,(SELECT COUNT(*) FROM lessons)::int AS lessons,(SELECT COUNT(*) FROM plans)::int AS plans,(SELECT COUNT(*) FROM teachers)::int AS teachers,(SELECT COUNT(*) FROM classes)::int AS classes,(SELECT COUNT(*) FROM questions)::int AS questions');counts=rows[0];}catch(e){}}
...
app.use(async(req,res,next)=>{
  ...
  try{await ensureReady();next();}catch(e){res.status(503).json({error:'Database is temporarily unavailable. Please refresh in a few seconds.'});}
});
