...
if (hasPostgres) {
  ...
  const normalizedConnectionString = /pooler\.supabase\.com/i.test(databaseUrl) ? databaseUrl.replace(/:5432\/postgres(?=$|\?)/i, ':6543/postgres') : databaseUrl;
...
app.get('/api/health',async(q,r)=>{
  ...
  r.status(dbOk?200:503).json({ok:dbOk,service:'ScorePath',database:hasPostgres?'postgres':'local',productionReady:hasPostgres&&dbOk,databaseConfigured:hasPostgres,initialized:startupStarted&&!startupError,counts,error});
});
