...
if (hasPostgres) {
  ...
  // Supabase Transaction Pooler (6543) is much more reliable for Vercel/serverless.
  // If the user supplied a Supabase pooler URL on the session port, use the transaction port.
  const normalizedConnectionString = /pooler\.supabase\.com/i.test(databaseUrl) ? databaseUrl.replace(/:(?:5432|5433)\/postgres(?=$|\?)/i, ':6543/postgres') : databaseUrl;
  pgPool = new Pool({ connectionString: normalizedConnectionString, ssl: process.env.DATABASE_SSL === 'false' ? false : { rejectUnauthorized: false }, max: 1, min: 0, connectionTimeoutMillis: 10000, idleTimeoutMillis: 30000, query_timeout: 15000, statement_timeout: 15000, allowExitOnIdle: true });
...
const seed = {
  ...
  teachers: [
    {id:'t1',name:'Meera Sharma',bio:'IELTS & Academic English mentor focused on speaking, writing and practical exam strategy.',photoUrl:''},
    {id:'t2',name:'Arjun Kapoor',bio:'PTE specialist helping learners improve fluency, pronunciation and task strategy.',photoUrl:''},
    {id:'t3',name:'Naina Bhatia',bio:'Academic English and grammar coach for clear, confident university-level communication.',photoUrl:''}
  ],
  classes: [
    {id:'c1',title:'IELTS Speaking Live Lab',exam:'IELTS',date:'Every Monday',time:'7:00 PM IST',instructor:'Meera Sharma',teacherId:'t1',link:'',description:'Speaking practice, feedback and live Q&A.'},
    {id:'c2',title:'PTE Fluency & Pronunciation',exam:'PTE',date:'Every Wednesday',time:'7:00 PM IST',instructor:'Arjun Kapoor',teacherId:'t2',link:'',description:'Fluency drills and exam-focused speaking practice.'},
    {id:'c3',title:'Academic Writing Workshop',exam:'Academic',date:'Every Friday',time:'7:00 PM IST',instructor:'Naina Bhatia',teacherId:'t3',link:'',description:'Structure, coherence, grammar and feedback.'}
  ],
  courses: [
...
async function initPostgres(){
  ...
  // Idempotent seed/repair: add any built-in records that are missing, even if the database is partially populated.
...
async function auth(req,res,next){try{await ensureReady();const u=await currentUser(req);if(!u)return res.status(401).json({error:'Please log in.'});req.user=u;next();}catch(e){console.error(e);res.status(500).json({error:'Authentication service error.'});}}
async function admin(req,res,next){try{await ensureReady();const u=await currentUser(req);if(!u||u.role!=='admin')return res.status(403).json({error:'Admin access required.'});req.user=u;next();}catch(e){res.status(500).json({error:'Authentication service error.'});}}
...
// Static files live at the project root so GitHub/Vercel uploads cannot lose the public folder.
app.get('/app.js',(q,r)=>r.sendFile(path.join(PUBLIC_DIR,'app.js')));
app.get('/styles.css',(q,r)=>r.sendFile(path.join(PUBLIC_DIR,'styles.css')));
app.get('/favicon.svg',(q,r)=>r.sendFile(path.join(PUBLIC_DIR,'favicon.svg')));
app.get('/robots.txt',(q,r)=>r.type('text/plain').sendFile(path.join(PUBLIC_DIR,'robots.txt')));
app.get('/sitemap.xml',(q,r)=>r.type('application/xml').sendFile(path.join(PUBLIC_DIR,'sitemap.xml')));
app.get('/',(q,r)=>r.sendFile(path.join(PUBLIC_DIR,'index.html')));
let startupError=null;
let startupPromise=null;
let startupStarted=false;
let startupAt=0;
async function ensureReady(){
  if(!hasPostgres){startupError=null;startupStarted=true;return;}
  ...
  startupPromise=(async()=>{
    ...
    for(let attempt=1;attempt<=3;attempt++){
      try{
        ...
        await ensureAdmin();
        startupError=null;
        console.log(`ScorePath ready (PostgreSQL, ${Date.now()-startupAt}ms)`);
        return true;
      }catch(e){
        last=e; startupError=e;
        console.error(`Startup attempt ${attempt}/3 failed:`,e.message);
        if(attempt<3) await new Promise(r=>setTimeout(r,700*attempt));
      }
    }
    throw last;
  })().catch(e=>{startupError=e;startupPromise=null;throw e;});
  return startupPromise;
}
async function tryReady(){try{await ensureReady();return true;}catch{return false;}}
app.get('/api/health',async(q,r)=>{
  let dbOk=!hasPostgres, error=null, counts=null;
  if(hasPostgres){
    try{await ensureReady(); await sql('SELECT 1'); dbOk=true;}catch(e){error={code:e.code||e.name||'DB_ERROR',message:'Database connection failed or initialization failed. Check the Supabase Transaction Pooler DATABASE_URL.'};}
    if(dbOk){try{const rows=await sql('SELECT (SELECT COUNT(*) FROM courses)::int AS courses,(SELECT COUNT(*) FROM lessons)::int AS lessons,(SELECT COUNT(*) FROM plans)::int AS plans,(SELECT COUNT(*) FROM teachers)::int AS teachers,(SELECT COUNT(*) FROM classes)::int AS classes,(SELECT COUNT(*) FROM questions)::int AS questions');counts=rows[0];}catch(e){error={code:e.code||e.name||'DB_ERROR',message:'Database connected but the ScorePath tables could not be checked.'};dbOk=false;}}
  }
  r.status(dbOk?200:503).json({ok:dbOk,service:'ScorePath',database:hasPostgres?'postgres':'local',productionReady:hasPostgres&&dbOk,databaseConfigured:hasPostgres,initialized:startupStarted&&!startupError,counts,error,mode:hasPostgres?'database':'demo-fallback'});
});

app.get('/api/courses',async(q,r)=>{try{const d=await getAll();r.json((d.courses||[]).filter(x=>x.published!==false));}catch(e){console.error(e);r.status(500).json({error:'Could not load courses.'});}});
...
async function getContentData(){
  if(!hasPostgres)return localDb;
  const [courses,lessons,classes,teachers,plans,questions]=await Promise.all([
    sql('SELECT id,exam,title,description,skills,published FROM courses WHERE published=true ORDER BY title'),
    sql('SELECT id,course_id as "courseId",exam,skill,title,published FROM lessons WHERE published=true ORDER BY course_id,id'),
    sql('SELECT id,title,exam,date,time,instructor,teacher_id as "teacherId",link,description FROM classes ORDER BY id'),
    sql('SELECT id,name,bio,photo_url as "photoUrl" FROM teachers ORDER BY name'),
    sql('SELECT id,name,price,days,featured,course_ids as "courseIds",features,published FROM plans WHERE published=true ORDER BY price'),
    sql('SELECT id,test,prompt,options,answer FROM questions ORDER BY id').catch(()=>[])
  ]);
  for(const c of courses){c.skills=Array.isArray(c.skills)?c.skills:[];c.lessons=lessons.filter(l=>l.courseId===c.id).length;c.firstLessonId=lessons.find(l=>l.courseId===c.id)?.id||'';}
  return {courses,lessons,classes,teachers,plans,pricing:plans,questions};
}
app.get('/api/content',async(q,r)=>{
  try{
    const ready=await tryReady();
    const d=ready?await getContentData():seed;
    const courses=(d.courses||[]).filter(x=>x.published!==false);
    const courseIds=new Set(courses.map(x=>x.id));
    r.setHeader('Cache-Control','s-maxage=60, stale-while-revalidate=300');
    r.json({courses,lessons:(d.lessons||[]).filter(x=>courseIds.has(x.courseId)&&x.published!==false).map(({body,objectives,...x})=>x),classes:d.classes||[],teachers:d.teachers||[],pricing:(d.plans||d.pricing||[]).filter(x=>x.published!==false),questions:d.questions||[],degraded:!ready});
  }catch(e){console.error(e);r.status(200).json({courses:seed.courses,lessons:seed.lessons.map(({body,objectives,...x})=>x),classes:seed.classes,teachers:seed.teachers,pricing:seed.plans,questions:seed.questions,degraded:true});}
});
app.get('/api/me',async(q,r)=>{try{await ensureReady();const u=await currentUser(q);r.json({user:u?{id:u.id,name:u.name,email:u.email,role:u.role}:null,subscription:u?await activeSub(u.id):null});}catch(e){r.status(500).json({error:'Could not load account.'});}});

app.post('/api/register',async(q,r)=>{try{await ensureReady();const{name,email,password}=q.body||{};if(!name||!email||!password||password.length<8)return r.status(400).json({error:'Enter your name, a valid email and a password of at least 8 characters.'});const e=email.trim().toLowerCase();let exists=false;if(hasPostgres)exists=(await sql('SELECT 1 FROM users WHERE email=$1',[e])).length>0;else exists=localDb.users.some(u=>u.email===e);if(exists)return r.status(409).json({error:'An account with this email already exists.'});const u={id:crypto.randomUUID(),name:name.trim(),email:e,role:'student',createdAt:new Date().toISOString()};const p=hashPassword(password);if(hasPostgres)await sql('INSERT INTO users(id,name,email,password_salt,password_hash,role) VALUES($1,$2,$3,$4,$5,$6)',[u.id,u.name,u.email,p.salt,p.hash,u.role]);else{u.password=p;localDb.users.push(u);localSave();}const token=crypto.randomBytes(32).toString('hex');if(hasPostgres)await sql("INSERT INTO sessions(token,user_id,expires_at) VALUES($1,$2,NOW()+INTERVAL '30 days')",[token,u.id]);else{localDb.sessions[token]={userId:u.id,expires:Date.now()+2592000000};localSave();}r.setHeader('Set-Cookie',`scorepath_session=${token}; ${cookieOpts()}`);r.json({ok:true});}catch(e){console.error(e);r.status(500).json({error:'Could not create your account.'});}});
app.post('/api/login',async(q,r)=>{try{await ensureReady();const{email,password}=q.body||{};const e=(email||'').trim().toLowerCase();let u;if(hasPostgres)u=(await sql('SELECT id,name,email,password_salt as salt,password_hash as hash,role FROM users WHERE email=$1',[e]))[0];else u=localDb.users.find(x=>x.email===e);if(!u||!verifyPassword(password||'',u.password||u))return r.status(401).json({error:'Incorrect email or password.'});const token=crypto.randomBytes(32).toString('hex');if(hasPostgres)await sql("INSERT INTO sessions(token,user_id,expires_at) VALUES($1,$2,NOW()+INTERVAL '30 days')",[token,u.id]);else{localDb.sessions[token]={userId:u.id,expires:Date.now()+2592000000};localSave();}r.setHeader('Set-Cookie',`scorepath_session=${token}; ${cookieOpts()}`);r.json({ok:true});}catch(e){console.error(e);r.status(500).json({error:'Could not log in.'});}});
app.post('/api/logout',async(q,r)=>{try{await ensureReady();const m=(q.headers.cookie||'').match(/scorepath_session=([^;]+)/);if(m){if(hasPostgres)await sql('DELETE FROM sessions WHERE token=$1',[m[1]]);else{delete localDb.sessions[m[1]];localSave();}}r.setHeader('Set-Cookie','scorepath_session=; HttpOnly; SameSite=Lax; Path=/; Max-Age=0');r.json({ok:true});}catch(e){r.status(500).json({error:'Could not log out.'});}});
...
if(require.main===module){ensureReady().catch(()=>{}).finally(()=>app.listen(PORT,()=>console.log(`Listening on ${PORT}`)));}
module.exports=app;
