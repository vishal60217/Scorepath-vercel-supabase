...
if (hasPostgres) {
  ...
  pgPool = new Pool({ connectionString: normalizedConnectionString, ssl: process.env.DATABASE_SSL === 'false' ? false : { rejectUnauthorized: false }, max: 3, min: 0, connectionTimeoutMillis: 15000, idleTimeoutMillis: 30000, query_timeout: 15000, statement_timeout: 15000, keepAlive: true, keepAliveInitialDelayMillis: 10000, allowExitOnIdle: true });
...
app.get('/api/health',async(q,r)=>{
  ...
  if(hasPostgres){
    try{await sql('SELECT 1');dbOk=true;}catch(e){error={code:e.code||e.name||'DB_ERROR',message:'Database connection failed. Check the Supabase connection string/pooler settings.'};}
    if(dbOk){try{const rows=await sql('SELECT (SELECT COUNT(*) FROM courses)::int AS courses,(SELECT COUNT(*) FROM lessons)::int AS lessons,(SELECT COUNT(*) FROM plans)::int AS plans,(SELECT COUNT(*) FROM teachers)::int AS teachers,(SELECT COUNT(*) FROM classes)::int AS classes,(SELECT COUNT(*) FROM questions)::int AS questions');counts=rows[0];}catch(e){}}
  }
