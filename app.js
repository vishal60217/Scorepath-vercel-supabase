...
async function init() {
  ...
  try { content = await api('/api/content'); }
  catch (e) { content = { courses: [], lessons: [], classes: [], pricing: [], questions: [] }; toast('The interface loaded, but live data is temporarily unavailable.', 'error'); }
  renderCourses(); renderClasses(); renderTest(); renderPlans();
  try { await updateAccount(); } catch (_) { const b = $('#accountBtn'); if (b) { b.textContent = 'Login'; b.onclick = () => openLogin(); } }
